...
type MockVerifier struct {
	...
	VerifyFn func(ctx context.Context, c types.Challenge, status int, contentType string, body []byte) (types.Verdict, error)
}

var _ types.Verifier = (*MockVerifier)(nil)

func (m *MockVerifier) Verify(ctx context.Context, c types.Challenge, status int, contentType string, body []byte) (types.Verdict, error) {
	m.mu.Lock()
	m.Calls = append(m.Calls, VerifierCall{Challenge: c})
	m.mu.Unlock()

	if m.VerifyFn != nil {
		return m.VerifyFn(ctx, c, status, contentType, body)
	}
